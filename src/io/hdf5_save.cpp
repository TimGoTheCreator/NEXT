...
static void WritePartTypeGroup(hid_t file, const std::string& group_name, const ParticleSystem& ps,
                               const std::vector<size_t>& indices, hid_t h5_real_type) {
    ...
    // Coordinates
    hid_t dset_coords =
        H5Dcreate(group, "Coordinates", H5T_NATIVE_FLOAT, space3, H5P_DEFAULT, dcpl, H5P_DEFAULT);
    ...
    // ParticleIDs
    hid_t dset_ids =
        H5Dcreate(group, "ParticleIDs", H5T_NATIVE_INT, space1, H5P_DEFAULT, dcpl, H5P_DEFAULT);
...
void SaveHDF5(const ParticleSystem& ps, const std::string& filename) {
    ...
    if (header_group >= 0) {
        int num_part_this_file[6] = {0, static_cast<int>(dm_indices.size()),    0,
                                     0, static_cast<int>(stars_indices.size()), 0};
        unsigned int num_part_total[6] = {0, static_cast<unsigned int>(dm_indices.size()),    0,
                                          0, static_cast<unsigned int>(stars_indices.size()), 0};
        ...
        hid_t attr;
        attr = H5Acreate(header_group, "NumPart_ThisFile", H5T_NATIVE_INT, space6, H5P_DEFAULT,
                         H5P_DEFAULT);
        H5Awrite(attr, H5T_NATIVE_INT, num_part_this_file);
        H5Aclose(attr);

        attr = H5Acreate(header_group, "NumPart_Total", H5T_NATIVE_UINT, space6, H5P_DEFAULT,
                         H5P_DEFAULT);
        H5Awrite(attr, H5T_NATIVE_UINT, num_part_total);
        H5Aclose(attr);

        attr = H5Acreate(header_group, "MassTable", H5T_NATIVE_DOUBLE, space6, H5P_DEFAULT,
                         H5P_DEFAULT);
        ...
        attr = H5Acreate(header_group, "Redshift", H5T_NATIVE_DOUBLE, space1, H5P_DEFAULT,
                         H5P_DEFAULT);
        H5Awrite(attr, H5T_NATIVE_DOUBLE, &redshift);
        H5Aclose(attr);

        attr = H5Acreate(header_group, "NumFilesPerSnapshot", H5T_NATIVE_INT, space1, H5P_DEFAULT,
                         H5P_DEFAULT);
    ...
    xmf << "<?xml version=\"1.0\" ?>\n<Xdmf Version=\"3.0\">\n  <Domain>\n";
    xmf << "    <Grid Name=\"ParticleCollection\" GridType=\"Collection\" "
    ...
    auto write_grid = [&](const std::string& name, const std::string& group, size_t count) {
        ...
        xmf << "          <DataItem Dimensions=\"" << count
            << "\" NumberType=\"Float\" Precision=\"" << precision << "\" Format=\"HDF\">\n";
 ...
 */
void SaveHDF5SingleStep(const ParticleSystem& ps, const std::string& filename, int step,
                        double time) {
    const size_t totalN = ps.size();
    if (totalN == 0) return;
    ...
    if (file < 0) {
        std::cerr << "[NEXT HDF5 Error] Failed to create or open file: " << filename << std::endl;
        return;
    }
    ...
    if (snap_group >= 0) {
        ...
        hid_t space1 = H5Screate_simple(1, dims1, NULL);
        hid_t attr =
            H5Acreate(snap_group, "Time", H5T_NATIVE_DOUBLE, space1, H5P_DEFAULT, H5P_DEFAULT);
        if (attr >= 0) {
            H5Awrite(attr, H5T_NATIVE_DOUBLE, &time);
            H5Aclose(attr);
        }
    ...
    for (const auto& entry : time_history) {
        ...
        auto write_grid = [&](const std::string& name, const std::string& group, size_t count) {
            ...
            xmf << "          <Attribute Name=\"Mass\" AttributeType=\"Scalar\" Center=\"Node\">\n";
            xmf << "            <DataItem Dimensions=\"" << count
                << "\" NumberType=\"Float\" Precision=\"" << ((sizeof(real) == 4) ? 4 : 8)
                << "\" Format=\"HDF\">\n";
